...
{
  v2portable::Player p;
  if (p.open(song.data(), song.size()) != v2portable::Result::OK) exit(2);
  p.play(0);
...
int main(int argc, char **argv)
{
  ...
  // interleaved
  v2portable::Player pa, pb;
  if (pa.open(a.data(), a.size()) != v2portable::Result::OK) return 2;
  if (pb.open(b.data(), b.size()) != v2portable::Result::OK) return 2;
  pa.play(0); pb.play(0);
