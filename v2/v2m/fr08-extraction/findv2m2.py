...
def try_parse(d,o):
    ...
    try:
        ...
        p=o+12+10*gdnum
        active=0; notes_total=0
        for ch in range(16):
            if p+4>n: return None
            notenum=u32(d,p); p+=4
            if notenum:
                if notenum>500000: return None
                active+=1; notes_total+=notenum
                p+=5*notenum
                if p+4>n: return None
                pcnum=u32(d,p); p+=4
        ...
        if p+4>n:return None
        psize=u32(d,p)
        if not(500<=psize<=1048576): return None  # patch bank must be substantial
        p+=4+psize
        if p+4>n:return None
        spsize=u32(d,p); p+=4
        ...
        return dict(start=o,end=p,size=p-o,timediv=timediv,maxtime=maxtime,
                    gdnum=gdnum,active=active,notes=notes_total,gsize=gsize,psize=psize,spsize=spsize)
    except Exception:
        return None
...
hits=[]
for o in range(0,len(d)-12,1):  # byte granularity, blob may be unaligned
    r=try_parse(d,o)
    if r and r['size']>=2000:
        hits.append(r)
# de-overlap: sort by start, keep, but show distinct
hits.sort(key=lambda h:-h['size'])
print('candidates:',len(hits))
for h in hits[:15]:
    print('off=%#x size=%d timediv=%d maxtime=%d gdnum=%d active=%d notes=%d gsize=%d psize=%d spsize=%d'%(
        h['start'],h['size'],h['timediv'],h['maxtime'],h['gdnum'],h['active'],h['notes'],h['gsize'],h['psize'],h['spsize']))
