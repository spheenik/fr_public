# v2-component-equivalence Specification

## Purpose

Localize divergence between the original V2 assembly core (`synth.asm`) and the C++ port (`synth_core.cpp`) at the granularity of individual DSP blocks, so that a whole-song mismatch can be traced to the specific block (or to composition/routing) responsible, by exercising each block in isolation on both cores under identical state, parameters, and input.
