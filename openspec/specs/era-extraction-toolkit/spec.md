# era-extraction-toolkit Specification

## Purpose
The shared offline toolkit for the period-binary hunt: turn a packed/unpacked period
farbrausch binary into its carved v2m song(s), its era-delta assay, and a reference
oracle render — packer detection/dispatch, v2m carving, era assay, disassembly,
static/live tapping, buffer-diff verification, and the native C oracle scaffold —
replacing the per-demo forked scripts with one canonical implementation (`v2/v2m/
toolkit/`, the `era` CLI + `oracle.h`). Migrations onto it are gated on existing
committed contracts (byte-identical carves/images, `max|d|=0` re-renders).
